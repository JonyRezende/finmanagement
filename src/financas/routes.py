...
Roteamento fino; as regras de negócio vivem em src/financas/domains/.
"""
import json
import os
import time
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/data":
            ...
            if not email:
                ...
                return
            data = firestore.get_user_data(email) or {"recurrences": [], "oneOffs": []}
    ...
    def _handle_register(self):
        ...
        firestore.save_user_data(email, {"recurrences": [], "oneOffs": []})
    ...
    def _handle_login(self):
        ...
        user = firestore.get_user_auth(email)
    ...
    def _handle_change_password(self):
        ...
        firestore.update_password(email, salt, password_hash)
    ...
    def _handle_change_email(self):
        ...
        if new_email == email:
            ...
            return
        if firestore.get_user_auth(new_email):
            self._send_json({"error": "Email já cadastrado"}, status=409)
            return

        firestore.rename_user(email, new_email)
        if token:
